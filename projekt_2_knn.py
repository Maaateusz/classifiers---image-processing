...
X = []
X = load_image_files(folder_train)
y = []
...
for k in range(len(knn_list)):
    ...
    liy.append(score)
    lix.append(knn_list[k])
    test_classes_predicted = neigh.predict(X_val)
    cm = confusion_matrix(y_val, test_classes_predicted)
    print("| ", cm)
    time_end = time.time() - time_start
    print("| Czas All: %.2f sec" % (time_end))
    print("|___")
...
x_pos = [i for i, _ in enumerate(lix)]
plt.bar(x_pos, liy, color='green')
plt.xticks(x_pos , lix)
plt.show()
